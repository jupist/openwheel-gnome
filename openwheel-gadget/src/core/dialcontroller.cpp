...
void DialController::setupConnections()
{
    ...
    // Forward focus-escape requests so the QML overlay can lower itself
    // before a keystroke is injected (e.g. zoom on Wayland).
    connect(m_actionExecutor.get(), &ActionExecutor::focusEscapeRequired,
            this, &DialController::focusEscapeRequired);
}
...
// ── Daemon autostart (systemd user service) ───────────────────────────────

int DialController::daemonAutostart() const
{
    // Ask systemd whether the unit is enabled. Exit code 0 = enabled.
    int rc = QProcess::execute(
        QStringLiteral("systemctl"),
        {QStringLiteral("--user"), QStringLiteral("is-enabled"),
         QStringLiteral("openwheel-daemon")});
    return (rc == 0) ? 1 : 0;
}

void DialController::setDaemonAutostart(int enabled)
{
    const QString verb = (enabled != 0)
                         ? QStringLiteral("enable")
                         : QStringLiteral("disable");
    QProcess::startDetached(
        QStringLiteral("systemctl"),
        {QStringLiteral("--user"), verb,
         QStringLiteral("openwheel-daemon")});
    Q_EMIT daemonAutostartChanged();
    qDebug() << "Daemon autostart:" << verb;
}
